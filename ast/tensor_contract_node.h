...
namespace udf
{
     ...
     * Implements tensor contractions (e.g., `t1 ** t2` for matrix multiplication-like operations).
     */
    ...
    public:
        tensor_contract_node(int lineno, cdk::expression_node *left, cdk::expression_node *right, const std::string &operation) : cdk::expression_node(lineno), _left(left), _right(right), _operation(operation)
        {
        }
