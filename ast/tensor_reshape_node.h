...
namespace udf
{
     ...
     * Manages operations that change the tensor's shape while preserving data (e.g., `t.reshape(3,2)`).
     */
    ...
    public:
        tensor_reshape_node(int lineno, cdk::expression_node *argument, cdk::expression_node *shape) : cdk::expression_node(lineno), _argument(argument), _shape(shape)
        {
        }
